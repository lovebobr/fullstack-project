  ...
  async loadProfile() {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(reservationsResponse.data)) {
          reservations = reservationsResponse.data.map((reservation: any) => ({
            ...
            tableNumber:
              ...
              "Н/Д",
            tableSeats:
              reservation.table?.seats || reservation.table?.table_seats || 0,
          ...
          reservations = reservationsResponse.data.data.map(
            (reservation: any) => ({
              id: reservation.id,
              restaurantName:
                reservation.table?.restaurant?.name ||
        ...
        console.log(
          "Пример данных бронирования с блюдами:",
          reservations[0]?.foods
        );
        ...
        try {
          ...
          console.log(
            "Ответ от /my-reservations:",
            myReservationsResponse.data
          );

          if (Array.isArray(myReservationsResponse.data)) {
            reservations = myReservationsResponse.data.map(
              (reservation: any) => ({
                id: reservation.id,
                restaurantName:
                  reservation.table?.restaurant?.name || "Ресторан",
                tableNumber:
                  reservation.table?.number ||
                  reservation.table_id?.toString() ||
                  "Н/Д",
      ...
      runInAction(() => {
        ...
        console.log("Профиль успешно загружен:", {
          ...
          foodsInFirstReservation: reservations[0]?.foods?.length || 0,
        });
      });
    } catch (error: any) {
